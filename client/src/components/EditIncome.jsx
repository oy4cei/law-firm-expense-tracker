...
export default function EditIncome() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [incomesRes, casesRes] = await Promise.all([
                    axios.get(`/api/incomes`),
                    axios.get('/api/cases')
                ]);

                const income = incomesRes.data.find(i => i.id === parseInt(id));
                if (income) {
                    setFormData({
                        ...
                        caseId: income.CaseId || '',
                        account: income.account || 'Cash'
                    });
                } else {
                    alert('Дохід не знайдено');
                    navigate('/incomes');
                }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await axios.put(`/api/incomes/${id}`, formData);
            navigate('/incomes');
        } catch (error) {
            console.error('Error updating income:', error);
            alert('Не вдалося оновити дохід');
        }
    };
    ...
    return (
        <div className="max-w-2xl mx-auto bg-white shadow rounded-lg p-6">
            <h2 className="text-xl font-bold mb-4 text-gray-900">Редагувати дохід</h2>
